...
TIP_INFO = CommandInfo(
    ...
    details = f"Tip specified amount to mentioned user(s) (**minimum tip is {Constants.TIP_MINIMUM} {Constants.TIP_UNIT}**)" +
     ...
     f"Example: `{config.Config.instance().command_prefix}{'ban' if Env.banano() else ('paw' if Env.paw() else 'ntip')} 2 @user1 @user2` would send 2 to user1 and 2 to user2"
)
TIPSPLIT_INFO = CommandInfo(
    triggers = ["bansplit", "bs"] if Env.banano() else (["pawsplit", "ps"] if Env.paw() else ["ntipsplit", "ns"]),
    ...
    details = f"Divide the specified amount between mentioned user(s) (**minimum tip is {Constants.TIP_MINIMUM} {Constants.TIP_UNIT}**)" +
        "\nThe recipient(s) will be notified of your tip via private message" +
        "\nSuccessful tips will be deducted from your available balance immediately.\n" +
     f"Example: `{config.Config.instance().command_prefix}{'bansplit' if Env.banano() else ('pawsplit' if Env.paw() else 'ntipsplit')} 2 @user1 @user2` would send 1 to user1 and 2 to user2"
)
TIPRANDOM_INFO = CommandInfo(
    triggers = ["banrandom", "br"] if Env.banano() else (["ptiprandom", "ptr"] if Env.paw() else ["ntiprandom", "ntr"]),
