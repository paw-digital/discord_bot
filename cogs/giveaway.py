...
TIPGIVEAWAY_INFO = CommandInfo(
    triggers = ["donate", "do"] if Env.banano() else (["ptipgiveaway", "ptg"] if Env.paw() else ["ntipgiveaway", "ntg"]),
    ...
    details = "Donate to the currently active giveaway to increase the pot, or donate to towards starting a giveaway automatically." +
                f"\nExample: `{config.Config.instance().command_prefix}{'donate' if Env.banano() else ('ptipgiveaway' if Env.paw() else 'ntipgiveaway')} 1` - Donate 1 {Env.currency_symbol()} to the current or next giveaway"
...
class GiveawayCog(commands.Cog):
    ...
    def format_giveaway_announcement(self, giveaway: Giveaway, amount: int = None) -> discord.Embed:
        ...
        if fee > 0:
            ...
            embed.description+= f"\n`{config.Config.instance().command_prefix}{'donate' if Env.banano() else ('ptipgiveaway' if Env.paw() else 'ntipgiveaway')} <amount>` - To increase the pot"
        else:
